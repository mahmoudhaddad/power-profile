...
class BuildingComponentController extends Controller
{
    ...
    {
        if (! $this->getRole($request, $building)) {
            return response()->json(['message' => 'Forbidden.'], 403);
        }

        return response()->json([
            'data' => $building->components()->with('componentType')->orderBy('created_at', 'desc')->get(),
        ]);
    }

    public function store(Request $request, Building $building)
    {
        if (! in_array($this->getRole($request, $building), ['admin', 'main'])) {
            return response()->json(['message' => 'Forbidden.'], 403);
        }
        ...
        $component = $building->components()->create([
    ...
    public function update(Request $request, Building $building, BuildingComponent $component)
    {
        if (! in_array($this->getRole($request, $building), ['admin', 'main'])
            || $component->building_id !== $building->id) {
            return response()->json(['message' => 'Forbidden.'], 403);
        }
    ...
    public function destroy(Request $request, Building $building, BuildingComponent $component)
    {
        if (! in_array($this->getRole($request, $building), ['admin', 'main'])
            || $component->building_id !== $building->id) {
            return response()->json(['message' => 'Forbidden.'], 403);
