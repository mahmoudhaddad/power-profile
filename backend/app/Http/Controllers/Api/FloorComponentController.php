...
class FloorComponentController extends Controller
{
    ...
    {
        if (! $this->getRole($request, $floor)) {
            return response()->json(['message' => 'Forbidden.'], 403);
        }

        return response()->json([
            'data' => $floor->components()->with('componentType')->orderBy('created_at', 'desc')->get(),
        ]);
    }

    public function store(Request $request, Floor $floor)
    {
        if (! in_array($this->getRole($request, $floor), ['admin', 'main'])) {
            return response()->json(['message' => 'Forbidden.'], 403);
        }
        ...
        $component = $floor->components()->create([
    ...
    public function update(Request $request, Floor $floor, FloorComponent $component)
    {
        if (! in_array($this->getRole($request, $floor), ['admin', 'main'])
            || $component->floor_id !== $floor->id) {
            return response()->json(['message' => 'Forbidden.'], 403);
        }
    ...
    public function destroy(Request $request, Floor $floor, FloorComponent $component)
    {
        if (! in_array($this->getRole($request, $floor), ['admin', 'main'])
            || $component->floor_id !== $floor->id) {
            return response()->json(['message' => 'Forbidden.'], 403);
