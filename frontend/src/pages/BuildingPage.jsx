...
export default function BuildingPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      api.get('/api/component-types'),
        ...
        const bld = floorsRes.data.building;
        setBuilding(bld);
        setProject(bld.project);
        setNameInput(bld.name);
        setFloors(floorsRes.data.data);
        setComponentTypes(typesRes.data.data);
        api.get(`/api/projects/${bld.project.id}/all-floors`)
  ...
  async function handleDuplicateFromSuggestion(floorId) {
    const { data } = await api.post(
      `/api/buildings/${building.id}/floors/${floorId}/duplicate`
    );
    setFloors(prev => [data.data, ...prev]);
    setShowModal(false);
          ...
          {editingName ? (
            <div className="flex items-center gap-2 mt-0.5">
              ...
              <button onClick={() => setEditingName(false)}
                className="text-xs text-ink-muted hover:text-ink-body font-medium px-2 py-1 rounded hover:bg-surface-inset transition-colors">Cancel</button>
            </div>
          ) : (
            <div className="flex items-center gap-2 mt-0.5">
        ...
        <section>
          <div className={`flex items-center justify-between ${openFloors ? 'mb-4' : 'mb-0'}`}>
            <button onClick={() => setOpenFloors(o => !o)} className="flex items-center gap-2 group">
              <svg className={`w-4 h-4 text-ink-muted transition-transform duration-200 ${openFloors ? 'rotate-90' : ''}`}
                fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M9 5l7 7-7 7" />
              </svg>
              <h2 className="text-base font-semibold text-ink-heading group-hover:text-ink-body2">
                Floors
                <span className="ml-2 text-xs font-normal text-ink-muted">({floors.length})</span>
              </h2>
            </button>
          </div>
          {openFloors && <div className="bg-surface-card rounded-xl border border-line overflow-hidden">
            {floors.length === 0 ? (
        ...
        {canEdit && (
          <aside className="w-72 flex-shrink-0 flex flex-col gap-4">
            <div className="bg-surface-card rounded-xl border border-line p-4">
              <h2 className="text-sm font-semibold text-ink-heading mb-3">Add Floor</h2>
              <button onClick={() => setShowModal(true)}
                className="group flex items-center gap-3 border-2 border-dashed border-accent-border
                  ...
                  rounded-xl px-4 py-3 w-full transition-all duration-200">
                ...
                <div className="text-left">
                  ...
                  <p className="text-xs text-accent-light group-hover:text-accent-lighter transition-colors">Add a floor to this building</p>
                </div>
              </button>
            </div>

            <div className="bg-surface-card rounded-xl border border-line p-4">
              <h2 className="text-sm font-semibold text-ink-heading mb-3">Restore Floor</h2>
              <div className="flex gap-1 mb-3 bg-surface-inset rounded-lg p-1">
                <button onClick={() => setRestoreTab('computer')}
                  className={`flex-1 px-2 py-1.5 text-xs font-medium rounded-md transition-colors ${restoreTab === 'computer' ? 'bg-surface-card text-ink-heading' : 'text-ink-body2 hover:text-ink-heading'}`}>
                  Computer
                </button>
                <button onClick={() => setRestoreTab('server')}
                  className={`flex-1 px-2 py-1.5 text-xs font-medium rounded-md transition-colors ${restoreTab === 'server' ? 'bg-surface-card text-ink-heading' : 'text-ink-body2 hover:text-ink-heading'}`}>
                  Server
                </button>
              </div>
              {restoreTab === 'computer' ? (
                    ...
                    <div className="mt-2 flex items-center justify-between bg-accent-soft border border-accent-border rounded-xl px-3 py-2">
                      ...
                      <div className="flex items-center gap-1.5 ml-2 flex-shrink-0">
                        <button onClick={() => { setRestoreFile(null); setRestoreError(null); if (fileInputRef.current) fileInputRef.current.value = ''; }}
                          className="text-xs text-ink-muted hover:text-ink-body px-1.5 py-0.5 rounded hover:bg-surface-inset transition-colors">Clear</button>
      ...
      {confirmData && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <div className="bg-surface-card border border-line rounded-2xl shadow-2xl w-full max-w-sm p-6">
            ...
            <h3 className="text-base font-semibold text-ink-heading text-center mb-2">Floor Already Exists</h3>
...
function FloorRow({ floor, canEdit, onOpen, onEdit, onDelete, onBackup, onDuplicate }) {
  return (
    <div onClick={onOpen} className="flex items-center justify-between px-5 py-4 border border-line rounded-xl
      cursor-pointer group transition-all duration-200 hover:bg-surface-inset hover:border-line-strong hover:-translate-y-1">
      ...
      <div className="flex items-center gap-1.5 w-28 text-sm text-ink-body2">
      ...
      <div className="bg-surface-card border border-line rounded-2xl shadow-2xl w-full max-w-sm p-6">
        ...
        <div className="space-y-4">
          <div className="relative" ref={wrapperRef}>
            ...
            {showSuggestions && filtered.length > 0 && (
              <ul className="absolute z-10 w-full bg-surface-card border border-line rounded-lg shadow-lg mt-1 max-h-48 overflow-y-auto">
                {filtered.map((s, i) => (
        ...
        <div className="flex gap-3 mt-6">
          ...
          <button onClick={onSubmit} disabled={!form.name.trim() || Number(form.area) <= 0}
            className="flex-1 bg-accent-gradient text-base py-2.5 rounded-lg text-sm font-medium hover:shadow-accent transition-shadow disabled:opacity-40 disabled:cursor-not-allowed">
            {submitLabel}
          </button>
        </div>
      </div>
